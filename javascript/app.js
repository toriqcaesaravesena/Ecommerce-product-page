...
const   nextBtnMob              = document.querySelector(".slider-mob-next-btn");
const   prevBtnMob              = document.querySelector(".slider-mob-prev-btn");
const   incQtyBtn               = document.querySelector(".product-buy-increase-icon");
const   decQtyBtn               = document.querySelector(".product-buy-decrease-icon");
const   inpQty                  = document.getElementById("product-quantity");
const   addToCartBtn            = document.querySelector(".product-buy-add-cart");
const   cartContent             = document.querySelector(".cart-content");
const   desktopSliderThumbs     = Array.from(document.querySelectorAll('.slider-desktop-thumb'));
const   desktopSliderImg        = document.querySelector(".slider-desktop-active-photo img");
const   lightBoxSliderImg       = document.querySelector(".slider-lightbox-active-photo img");
const   closeLightBox           = document.querySelector(".slider-lightbox-close-icon");
const   lightBoxSliderThumbs    = Array.from(document.querySelectorAll('.slider-lightbox-thumb'));
const   nextBtnLightBox         = document.querySelector(".slider-lightbox-next-btn");
const   prevBtnLightBox         = document.querySelector(".slider-lightbox-prev-btn");
let     emptyCartBtn;
...
menuIcon.addEventListener("click", () => {
    alterClass(document.querySelector(".nav-menu"), "nav-menu--visible", "add");
    addOverlay();
});
// Hide menu Icon When click on Close Icon
closeIcon.addEventListener("click", () => {
    alterClass(document.querySelector(".nav-menu"), "nav-menu--visible", "remove")
    removeOverlay();
});
...
desktopSliderThumbs.forEach(element => {
    element.addEventListener("click", function(){
        showClickedPhoto(element, desktopSliderThumbs, desktopSliderImg, "slider-desktop-thumb--active");
        sliderTracker(element, lightBoxSliderThumbs, lightBoxSliderImg, "slider-lightbox-thumb--active");
    });
});
...
lightBoxSliderThumbs.forEach(element => {
    element.addEventListener("click", function(){
        showClickedPhoto(element, lightBoxSliderThumbs, lightBoxSliderImg, "slider-lightbox-thumb--active");
        sliderTracker(element, desktopSliderThumbs, desktopSliderImg, "slider-desktop-thumb--active");
    });
});
// Hide Light Box
...
function slide(slideTo){
    const sliderBar         = document.querySelector(".slider-mob-bar");
    const slideWidth        = document.querySelector('.slider-mob-item').offsetWidth;
    const sliderItemsNum    = Array.from(document.querySelectorAll(".slider-mob-item"))
    if(slideTo === "next"){
...
function addToCart(){
    ...
    if(typeof qtyValue === "number" && qtyValue > 0){
        ...
        <h3 class="cart-content-heading">Cart</h3>
        <div class="cart-content-full">
            <div class="cart-content-product">
                <div class="cart-content-prod-img"><img src="images/image-product-1-thumbnail.jpg" alt="Product"></div>
                <div class="cart-content-prod-text">
                    <h6>Autumn Limited Edition...</h6>
                    <div class="cart-content-prod-price">$125.00 * ${qtyValue} <span>$${125.00 * qtyValue}</span></div>
                </div>
                <div class="cart-content-bin-icon">
        ...
        emptyCartBtn    = document.querySelector(".cart-content-bin-icon");
        if(emptyCartBtn){emptyCartBtn.addEventListener("click", emptyCart)};
    }else{
...
function lightboxSlide(slideTo){
    lightBoxSliderThumbs.forEach((e, i) => {
        if(e.classList.contains("slider-lightbox-thumb--active")){
            currentImgIndex = i;
        }
    });
    if(slideTo === "next"){
        if(currentImgIndex < lightBoxSliderThumbs.length - 1){
            lightBoxSliderThumbs[currentImgIndex].classList.remove("slider-lightbox-thumb--active");
            currentImgIndex++;
            lightBoxSliderImg.setAttribute("src", lightBoxSliderThumbs[currentImgIndex].getAttribute("data-src"));
            lightBoxSliderThumbs[currentImgIndex].classList.add("slider-lightbox-thumb--active");
            sliderTracker(lightBoxSliderThumbs[currentImgIndex], desktopSliderThumbs, desktopSliderImg, "slider-desktop-thumb--active");
        }
    }else if(slideTo === "previous"){
